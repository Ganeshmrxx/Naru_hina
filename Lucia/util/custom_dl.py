...
class ByteStreamer:
    ...
    async def get_file_properties(self, msg_id: int, channel_id: int) -> FileId:
        cache_key = f"{channel_id}:{msg_id}"

        if cache_key not in self.cached_file_ids:
            await self.generate_file_properties(msg_id, channel_id)

        return self.cached_file_ids[cache_key]

    async def generate_file_properties(self, msg_id: int, channel_id: int) -> FileId:
    ...
    async def clean_cache(self):
        while True:
            ...
            logging.debug("ByteStreamer cache cleared")

    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            btn= [[
                ...
                InlineKeyboardButton("𝖣𝗈𝗐𝗇𝗅𝗈𝖺𝖽", url=f"https://t.me/ipapkorn_v1_bot?start=file_{query.message.chat.id}_{file_id}")        
	    ]]
            await query.edit_message_reply_markup(
                reply_markup=InlineKeyboardMarkup(btn)
	    )
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings.get('button'):
        btn = [
            [
                InlineKeyboardButton(
                    text=f"{silent_size(file.file_size)}| {extract_tag(file.file_name)} {clean_filename(file.file_name)}", callback_data=f'streamfile:{file.file_id}'
                ),
            ]
            for file in files
        ]
